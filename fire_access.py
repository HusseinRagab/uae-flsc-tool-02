...
def _extent_midrise_nonsprinklered(footprint: float) -> Tuple[str, str]:
    if footprint < 2000:
        return "Minimum 1/6 perimeter (at least 15 m)", "Table 2.3 < 2,000 m2"
    if footprint <= 4000:
        return "1/4 perimeter", "Table 2.3 2,000-4,000 m2"
    if footprint <= 8000:
        return "1/2 perimeter", "Table 2.3 4,001-8,000 m2"
    if footprint <= 16000:
        return "3/4 perimeter", "Table 2.3 8,001-16,000 m2"
    return "Whole perimeter (100%)", "Table 2.3 > 16,000 m2"

def _extent_mid_high_sprinklered(footprint: float) -> Tuple[str, str]:
    if footprint < 4000:
        return "Minimum 1/6 perimeter (at least 15 m)", "Table 2.4 < 4,000 m2"
    if footprint <= 8000:
        return "1/4 perimeter", "Table 2.4 4,001-8,000 m2"
    if footprint <= 16000:
        return "1/2 perimeter", "Table 2.4 8,001-16,000 m2"
    if footprint <= 32000:
        return "3/4 perimeter", "Table 2.4 16,001-32,000 m2"
    return "Whole perimeter (100%)", "Table 2.4 > 32,000 m2"

def _extent_industrial(volume: float, sprinklered: bool) -> Tuple[str, str]:
    if not sprinklered:
        if volume < 28400:
            return "Minimum 1/6 perimeter (at least 15 m)", "Table 2.6 < 28,400 m3"
        if volume <= 56800:
            return "1/4 perimeter", "Table 2.6 28,400-56,800 m3"
        if volume <= 85200:
            return "1/2 perimeter", "Table 2.6 56,801-85,200 m3"
        if volume <= 113600:
            return "3/4 perimeter", "Table 2.6 85,201-113,600 m3"
        return "Whole perimeter (100%)", "Table 2.6 > 113,600 m3"
    if volume < 56800:
        return "Minimum 1/6 perimeter (at least 15 m)", "Table 2.7 < 56,800 m3"
    if volume <= 85200:
        return "1/4 perimeter", "Table 2.7 56,801-85,200 m3"
    if volume <= 113600:
        return "1/2 perimeter", "Table 2.7 85,201-113,600 m3"
    if volume <= 170400:
        return "3/4 perimeter", "Table 2.7 113,601-170,400 m3"
    return "Whole perimeter (100%)", "Table 2.7 > 170,400 m3"

def compute_fire_access(b: Building, requires_wet_riser: bool = False) -> FireAccessSummary:
    spr = _sprinklered(b, requires_wet_riser)
    fp = _footprint_m2(b)
    notes: List[str] = [
        "Accessway min width 6 m; vertical clearance 4.5 m (Table 2.2).",
        "Max parking distance: 15 m from entrance, 18 m from breeching inlet.",
        "Max road grade 10%.",
    ]
    ...
    if is_super or is_mall:
        ...
        notes.append("Not less than Table 2.3 or 2.4 where greater.")
    elif is_storage:
        vol = _volume_m3(b)
        extent, ref = _extent_industrial(vol, spr)
        notes.append(f"Gross cubical extent proxy approx {vol:,.0f} m3.")
    elif is_low and not spr:
        extent, ref = "Provide fire access road to within 15 m of entrance", "Table 2.2 (lowrise)"
...
def evaluate_fire_access(b: Building, requires_wet_riser: bool = False) -> ChapterReport:
    ...
    items = [
        Requirement(system="Fire vehicle accessway width + clearance", status="required",
            ...
            detail="; ".join(s.notes[:3]), code_ref="Ch 2, Table 2.2", page_ref="p.207",
        ...
        Requirement(system="Extent of fire vehicle access around building", status="required",
            spec=s.extent,
            detail=f"Footprint approx {s.footprint_m2:,.0f} m2; sprinklered assumed: {s.sprinklered_assumed}. {s.extent_ref}.",
            code_ref="Ch 2", page_ref="p.213-215", source_rule="fsa_extent"),
    ...
    return ChapterReport(
        ...
        blocks=[SectionBlock(title="FSA - Vehicle access and extent (Ch 2)", items=items)],
